...
let internal webscraping_DPO pathToDir =  
    ...
    let stateReducer (state: State) (message: Messages) (action: Actions) (environment: Environment) =
        ...
        let dirList pathToDir = [ sprintf"%s\%s"pathToDir ODIS.Default.odisDir5 ]
        ...
        | DeleteOneODISDirectory ->                                     
                                    ...
                                    let myDeleteFunction x =  
                                        ...
                                        let dirInfo = new DirectoryInfo(pathToDir) |> Option.toGenerics (lazy (message.msgParam7 "Chyba v průběhu odstraňování starých JŘ DPO.")) (new DirectoryInfo(pathToDir))   
